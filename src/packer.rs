...
pub enum Packet {
	...
	HadnshakeResponse(usize), // Worker ID
...
pub trait PackImpl<T> where T: Packable {
	fn pack(data: T) -> Bytes;
	fn unpack(b: Bytes) -> T;
}

pub struct Packer<T> where T: Packable {
	phantom: std::marker::PhantomData<T>,
}
impl<T> Packer<T> where T: Packable {
	pub fn new() -> Self {
		Self {
...
impl Packer<Packet> {
	pub fn pack(data: Packet) -> Bytes {
		match data {
			...
			Packet::HadnshakeResponse(worker_id) => {
				let mut b = BytesMut::new();
				b.put_u32(1 + 4 + 4);
			...
			Packet::Control(data) => {
				let mut b = BytesMut::new();
				let mut byte_arr = Packer::<ControlPacket>::pack(data);
	...
	pub fn unpack(b: Bytes) -> Option<Packet> {
		...
		match b[0] {
			0x00 => Some(Packet::Handshake(u32::from_be_bytes([b[1], b[2], b[3], b[4]]) as usize)),
			0x01 => Some(Packet::HadnshakeResponse(u32::from_be_bytes([b[1], b[2], b[3], b[4]]) as usize)),
			0x02 => Some(Packet::Video(b.slice(1..))),
			0x03 => Some(Packet::Control(Packer::<ControlPacket>::unpack(b.slice(1..)))),
			_ => None
		}
	}
